#!/usr/bin/env python3
"""Deploy 13.4.4 Home composer secondary-row polish via SSH atomic mv."""
...
def main() -> int:
    ...
    verify = c.exec_command(
        ...
        f"grep -c yai-home-composer__secondary {REMOTE_REL}/templates/studio-shell.php; "
        f"grep -c 'color: #080808' {REMOTE_REL}/assets/css/home-bottom-composer.css; "
        f"grep -c 'justify-content: space-between' {REMOTE_REL}/assets/css/home-bottom-composer.css",
        timeout=20,
    ...
    print("13.4.4 in html", "13.4.4" in page)
    print("secondary row", "yai-home-composer__secondary" in page)

    css = urllib.request.urlopen(
        f"https://yooyland.com/wp-content/plugins/yooy-ai-studio/assets/css/home-bottom-composer.css?ver=13.4.4&cb={cb}",
        timeout=30,
    ).read().decode("utf-8", "replace")
    print("space-between secondary", ".yai-home-composer__secondary" in css and "space-between" in css)
    print("black generate", "color: #080808" in css)
    print("compact home pad", "padding-bottom: 118px" in css)
    return 0
