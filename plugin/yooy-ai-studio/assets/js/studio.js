(function () {
  ...
  /**
   * After WP login redirect back to Studio, resume remix/template/prompt
   * that guest started before auth (sessionStorage survives same-origin return).
   * @return {boolean} true when a studio resume was scheduled
   */
  function resumePendingAfterAuth() {
    if (!isLoggedIn()) return false;
    var pending = '';
    try {
      pending = sessionStorage.getItem('yoy_pending_after_auth') || '';
      sessionStorage.removeItem('yoy_pending_after_auth');
    } catch (e) { return false; }
    if (!pending) return false;

    var studio = 'image';
    try {
      studio = sessionStorage.getItem('yoy_home_studio') || 'image';
      var remix = sessionStorage.getItem('yoy_home_remix');
      if (remix) {
        var shell = JSON.parse(remix);
        if (shell && (shell.studio || shell.type)) {
          studio = (typeof STUDIO_ROUTE_MAP !== 'undefined' && STUDIO_ROUTE_MAP[shell.studio])
            ? STUDIO_ROUTE_MAP[shell.studio]
            : (shell.studio || shell.type || studio);
          if (String(studio).indexOf('-studio') !== -1) {
            studio = String(studio).replace(/-studio$/, '');
          }
        }
      }
    } catch (e2) { /* ignore */ }
  ...
  function launchFromHome() {
    ...
    try {
      if (prompt) sessionStorage.setItem('yoy_home_prompt', prompt);
      sessionStorage.setItem('yoy_home_original_prompt', prompt);
      if (attachment && (attachment.url || attachment.preview || attachment.type)) {
        sessionStorage.setItem('yoy_home_attachment', JSON.stringify(attachment));
      }
      sessionStorage.setItem('yoy_pending_after_auth', 'generate');
    } catch (preAuth) { /* ignore */ }
    if (!requireLogin()) return;
    try { sessionStorage.removeItem('yoy_pending_after_auth'); } catch (clr) { /* ignore */ }
    var presetCtx = '';
  ...
  function renderProjects(items) {
    ...
    if (!el) return;
    if (!items.length) {
      el.innerHTML = emptyBlock('📁', '아직 프로젝트가 없습니다', '작품을 묶어 이어서 작업할 프로젝트를 만들어 보세요.', '프로젝트 만들기', 'projects', true);
      return;
    }
  ...
  function workspaceHeroUrl(w) {
    if (!w) return '';
    if (window.YooYGalleryImage && typeof window.YooYGalleryImage.pickUrl === 'function') {
      return window.YooYGalleryImage.pickUrl(w, 'full') || window.YooYGalleryImage.pickUrl(w, 'large') || '';
    }
    return w.full_url || w.original_url || w.image_url || w.output_url || w.asset_url || w.url || w.thumbnail_url || '';
  }

  function workspaceThumbUrl(w) {
    if (!w) return '';
    if (window.YooYGalleryImage && typeof window.YooYGalleryImage.pickUrl === 'function') {
  ...
  function workspaceHeroImgHtml(w, className) {
    ...
    if (!src) return '';
    var srcset = w && w.srcset ? String(w.srcset) : '';
    var attrs = 'src="' + esc(src) + '" alt="" decoding="async" fetchpriority="high"';
    if (srcset) {
      attrs += ' srcset="' + esc(srcset) + '" sizes="(max-width: 900px) 100vw, min(1100px, 92vw)"';
    }
  ...
  function workspaceAssetCardMediaHtml(w) {
    ...
    var workId = w.id || w.gallery_id || '';
    return '<button type="button" class="yai-workspace-asset-thumb yai-workspace-asset-thumb--hq" data-ws-original-view="' + esc(workId) + '" aria-label="원본 보기">' +
      '<img ' + attrs + '>' +
    '</button>';
  }
  ...
  function renderWorks(items) {
    ...
    if (!items.length) {
      ...
      el.innerHTML = emptyBlock('🖼', '아직 작품이 없습니다', '만들거나 가져온 작품이 Gallery에 모입니다.', '이미지 생성하기', 'image');
      return;
    }
  ...
  function openOriginalImageViewer(work, listOpt) {
    if (!work || !window.YooYOriginalImageViewer) return false;
    var type = String(work.type || 'image').toLowerCase();
    if (type && type !== 'image') return false;
    var list = listOpt;
    if (!list) {
      var works = (workspaceCache && workspaceCache.works) || [];
      list = works.filter(function (w) {
        if (!w || w.asset_missing) return false;
        var t = String(w.type || 'image').toLowerCase();
        return !t || t === 'image';
      });
    }
    var idx = 0;
    for (var i = 0; i < list.length; i++) {
      if (String(list[i].id || list[i].gallery_id) === String(work.id || work.gallery_id)) {
        idx = i;
        break;
      }
    }
    window.YooYOriginalImageViewer.open({
      item: work,
      items: list.length > 1 ? list.map(function (x) { return { item: x }; }) : null,
      index: idx,
      title: work.title || work.display_title || ''
    });
    return true;
  }
  ...
  function openWorkPreview(work) {
    if (!work) return;
    var type = String(work.type || 'image').toLowerCase();
    if ((type === 'image' || type === '') && window.YooYOriginalImageViewer) {
      if (openOriginalImageViewer(work)) return;
    }
  ...
  function renderJobs(items) {
    ...
    if (!homeActivityCache.length) {
      el.innerHTML = emptyBlock('⚡', '아직 활동이 없습니다', '생성 기록이 여기에 표시됩니다.', '이미지 생성하기', 'image');
      return;
    }
  ...
  function renderWorkspaceOverview(project, works) {
    ...
    var main = recent[0];
    var coverHtml = main
      ? workspaceHeroImgHtml(main, 'yai-workspace-hero-image')
      : '';
    if (!coverHtml) {
      var fallback = projectCoverUrl(project, true);
      if (fallback) {
        coverHtml = '<img class="yai-workspace-hero-image" src="' + esc(fallback) + '" alt="" decoding="async" fetchpriority="high">';
      }
    }
      ...
      '<div class="yai-workspace-hero-actions">' +
        ...
        (mainId && !main.asset_missing
          ? '<button type="button" class="yai-btn yai-btn--gold yai-btn--sm" data-ws-asset-action="publish" data-work-id="' + esc(mainId) + '" data-gallery-id="' + esc(mainId) + '">공개하기</button>' +
            ...
            '<button type="button" class="yai-btn yai-btn--outline yai-btn--sm" data-ws-original-view="' + esc(mainId) + '">원본 보기</button>'
          : '') +
      '</div>' +
    ...
    Promise.all([
      ...
      if (assetRefs.length) {
        works = assetRefs.map(function (a) {
          ...
          var g = byId[gid] || {};
          var missing = !g.id;
          // Prefer live Gallery canonical URLs over cached Project asset thumbnails.
          return {
            id: gid,
            gallery_id: gid,
            type: a.type || g.type || 'image',
            title: a.title || g.title || 'Work',
            thumbnail_url: g.thumbnail_url || g.thumbnail || a.thumbnail || a.thumbnail_url || '',
            thumbnail: g.thumbnail_url || g.thumbnail || a.thumbnail || '',
            image_url: g.full_url || g.image_url || g.output_url || g.asset_url || a.url || '',
            output_url: g.output_url || g.full_url || a.url || '',
            full_url: g.full_url || g.original_url || g.image_url || a.url || '',
            original_url: g.original_url || g.full_url || '',
            large_url: g.large_url || g.full_url || '',
            medium_large_url: g.medium_large_url || '',
            asset_url: g.asset_url || g.full_url || a.url || '',
            srcset: g.srcset || '',
            sizes: g.sizes || '',
      ...
      // Enrich missing/partial refs via direct Gallery item fetch (canonical full URL + publishability).
      var needEnrich = works.filter(function (w) {
        return w && w.id && (w.asset_missing || !(w.full_url || w.image_url));
      }).slice(0, 12);

      var enrichPromise = needEnrich.length && Core.gallery && typeof Core.gallery.item === 'function'
        ? Promise.all(needEnrich.map(function (w) {
            return Core.gallery.item(w.id).then(function (res) {
              return (res.data && res.data.item) || null;
            }).catch(function () { return null; });
          }))
        : Promise.resolve([]);

      return enrichPromise.then(function (items) {
        items.forEach(function (g) {
          ...
          works = works.map(function (w) {
            ...
            return Object.assign({}, w, {
              asset_missing: false,
              title: w.title || g.title || 'Work',
              thumbnail_url: g.thumbnail_url || g.thumbnail || w.thumbnail_url,
              thumbnail: g.thumbnail_url || g.thumbnail || w.thumbnail,
              image_url: g.full_url || g.image_url || g.output_url || w.image_url,
              output_url: g.output_url || g.full_url || w.output_url,
              full_url: g.full_url || g.original_url || g.image_url || w.full_url,
              original_url: g.original_url || g.full_url || w.original_url,
              large_url: g.large_url || g.full_url || w.large_url,
              medium_large_url: g.medium_large_url || w.medium_large_url,
              asset_url: g.asset_url || g.full_url || w.asset_url,
              srcset: g.srcset || w.srcset || '',
              sizes: g.sizes || w.sizes || ''
            });
          });
        });
        return { project: project, works: works };
      });
    }).then(function (bundle) {
      var project = bundle.project;
      var works = bundle.works;
          ...
          unassigned.map(function (w) {
            return '<div class="yai-project-suggest-item">' +
              ...
              '<div class="yai-project-actions yai-recent-work-actions">' +
                ...
                '<button type="button" class="yai-btn yai-btn--gold yai-btn--sm" data-recent-action="add" data-work-id="' + esc(w.id) + '">프로젝트에 추가</button>' +
              '</div>' +
            '</div>';
          }).join('') +
        '</div>';
  ...
  function loadProjects() {
    ...
    if (!el) return;
    if (!isLoggedIn()) {
      el.innerHTML = emptyBlock('', '로그인이 필요합니다', '로그인 후 프로젝트를 관리할 수 있습니다.', '홈으로', 'home');
      return;
    }
  ...
  function publicSafeFullUrl(item) {
    if (!item) return '';
    if (window.YooYGalleryImage && typeof window.YooYGalleryImage.pickUrl === 'function') {
      return window.YooYGalleryImage.pickUrl(item, 'full')
        || window.YooYGalleryImage.pickUrl(item, 'large')
        || '';
    }
    return item.full_url || item.large_url || item.display_url || item.image_url || item.output_url || '';
  }

  function openPublicOriginalViewer(item) {
    if (!item || !window.YooYOriginalImageViewer) return false;
    var type = String(item.type || 'image').toLowerCase();
    if (type && type !== 'image') return false;
    var url = publicSafeFullUrl(item);
    if (!url) return false;
    // Public-safe payload only — no private prompt/project/provider metadata.
    var safeItem = {
  ...
  function publicDiscoverThumb(item) {
    ...
    var thumbInner = (window.YooYGalleryImage && typeof window.YooYGalleryImage.imgTag === 'function')
    ...
    return '<button type="button" class="yai-pub-card__thumb" data-pub-action="original" data-gallery-id="' +
      esc(item.gallery_id || item.id || '') + '" aria-label="원본 보기">' + thumbInner + '</button>';
  }
  ...
  function bindPublicFeedActions(el, items, kind) {
    ...
    el.querySelectorAll('[data-pub-action]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        if (!item) return;
        var act = btn.getAttribute('data-pub-action');
        if (act === 'remix') {
          remixPublicWork(item);
          return;
        }
        if (act === 'original') {
          if (!openPublicOriginalViewer(item)) {
  ...
  function openPublicDetail(item, kind) {
    ...
    host.innerHTML =
      '<div class="yai-pub-detail" tabindex="-1">' +
        ...
        '<button type="button" class="yai-pub-detail__media" data-pub-original aria-label="원본 보기">' +
          (window.YooYGalleryImage && typeof window.YooYGalleryImage.imgTag === 'function'
            ? window.YooYGalleryImage.imgTag(item, { size: 'large', className: 'yai-pub-card__img', lazy: false })
            : ('<img src="' + esc(publicSafeFullUrl(item) || '') + '" alt="">')) +
        '</button>' +
        ...
        '<div class="yai-pub-card__actions">' +
          ...
          '<button type="button" class="yai-btn yai-btn--outline" data-pub-original>원본 보기</button>' +
    ...
    host.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-pub-original]')) {
        openPublicOriginalViewer(item);
      }
    });
  ...
  function loadHistory() {
    ...
    Core.gallery.works().then(function (res) {
      ...
      if (!works.length) {
        el.innerHTML = emptyBlock('', '아직 기록이 없습니다', '생성·저장된 Gallery 작업이 여기에 시간순으로 표시됩니다.', '이미지 생성하기', 'image');
        return;
      }
      ...
      Core.router.generate(payload).then(function (res) {
        ...
        result.innerHTML = '<div class="yai-card"><strong>작성 완료</strong>' +
          ...
          '<div class="yai-writing-result-actions" style="display:flex;flex-wrap:wrap;gap:8px;margin-top:12px">' +
            ...
            (galleryId ? '<button type="button" class="yai-btn yai-btn--outline yai-btn--sm" id="yai-writing-gallery" data-gallery-id="' + esc(galleryId) + '">Gallery에서 보기</button>' : '') +
          '</div></div>';
        var again = result.querySelector('#yai-writing-again');
        if (again) again.addEventListener('click', function () { btn.click(); });
        var addProj = result.querySelector('#yai-writing-project');
        if (addProj && window.YooYStudioPickProject) {
          addProj.addEventListener('click', function () {
            window.YooYStudioPickProject(addProj.getAttribute('data-gallery-id'));
          });
        }
        var openGal = result.querySelector('#yai-writing-gallery');
        if (openGal) {
          openGal.addEventListener('click', function () {
            var gid = openGal.getAttribute('data-gallery-id');
            if (window.YooYStudioRoute) window.YooYStudioRoute('works');
            if (window.YooYGallery && typeof window.YooYGallery.openDetail === 'function') {
              window.YooYGallery.openDetail(gid);
            }
          });
        }
        if (Core.notifyGalleryUpdated) Core.notifyGalleryUpdated();
  ...
  document.addEventListener('click', function (e) {
    ...
    var wsOriginal = e.target.closest('[data-ws-original-view]');
    if (wsOriginal) {
      e.preventDefault();
      e.stopPropagation();
      var ovid = wsOriginal.getAttribute('data-ws-original-view') || '';
      var worksList = (workspaceCache && workspaceCache.works) || [];
      var ovItem = worksList.find(function (w) {
        return w && (String(w.id) === String(ovid) || String(w.gallery_id || '') === String(ovid));
      });
      if (ovItem && openOriginalImageViewer(ovItem)) return;
      if (ovid) {
        openWorkDetail(ovid);
        return;
      }
