...
final class YooY_Music_History {
    ...
    public function get(int $user_id, string $id): ?array {
        ...
        if ($item && ($item['studio'] ?? '') === 'music-studio') return $item;
        return null;
    }

    public function add(int $user_id, array $result): array {
        return $this->store->save($user_id, $result, 'music-studio');
    }

    public function clear(int $user_id): void {
        $this->store->clear($user_id, 'music-studio');
    }
}
